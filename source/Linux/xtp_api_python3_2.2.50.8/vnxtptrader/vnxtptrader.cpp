...
void TraderApi::OnUnknownOrder(uint64_t order_xtp_id, uint64_t session_id)
{
	...
	task->addtional_int = order_xtp_id;
	task->addtional_int_two = session_id;
	this->task_queue.push(task);
};
...
{
	PyLock lock;
	this->onUnknownOrder(task->addtional_int, task->addtional_int_two);
	delete task;
}
...
bool TraderApi::setMaxOrderBufferQuantity(uint32_t max_order_qty)
{
	return this->api->SetMaxOrderBufferQuantity(max_order_qty);
};
...
BOOST_PYTHON_MODULE(vnxtptrader)
{
	...
	class_<TraderApiWrap, boost::noncopyable>("TraderApi")
		...
		.def("setMaxOrderBufferQuantity", &TraderApiWrap::setMaxOrderBufferQuantity)
