...
void QuoteApi::createQuoteApi(int clientid, char * path, int log_level)
{
	this->api = XTP::API::QuoteApi::CreateQuoteApi(clientid, path, (XTP_LOG_LEVEL)log_level);
...
int QuoteApi::queryAllTickersFullInfo(int exchange) {
	return this->api->QueryAllTickersFullInfo((XTP_EXCHANGE_TYPE) exchange);
}
