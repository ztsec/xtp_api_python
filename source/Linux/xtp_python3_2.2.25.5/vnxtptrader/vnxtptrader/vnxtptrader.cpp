...
void TraderApi::processOptionCombinedOrderEvent(Task *task) {
	...
	this->onOptionCombinedOrderEvent(data, error);
}
...
void TraderApi::processOptionCombinedTradeEvent(Task *task) {
	...
	this->onOptionCombinedTradeEvent(data);
}
...
void TraderApi::processQueryOptionCombinedOrders(Task *task) {
	...
	this->onQueryOptionCombinedOrders(data, error, task->task_id, task->task_last);
}
...
void TraderApi::processQueryOptionCombinedOrdersByPage(Task *task) {
	...
	this->onQueryOptionCombinedOrdersByPage(data, task->req_count, task->order_sequence, task->query_reference, task->task_id, task->task_last);
}
...
void TraderApi::processQueryOptionCombinedTrades(Task *task) {
	...
	this->onQueryOptionCombinedTrades(data, error, task->task_id, task->task_last);
}
...
void TraderApi::processQueryOptionCombinedTradesByPage(Task *task) {
	...
	this->onQueryOptionCombinedTradesByPage(data, task->req_count, task->trade_sequence, task->query_reference, task->task_id, task->task_last);
}
...
void TraderApi::processQueryOptionCombinedPosition(Task *task) {
	...
	this->onQueryOptionCombinedPosition(data, error, task->task_id, task->task_last);
}
...
void TraderApi::processQueryOptionCombinedStrategyInfo(Task *task) {
	...
	this->onQueryOptionCombinedStrategyInfo(data, error, task->task_id, task->task_last);
}
...
void TraderApi::processCancelOptionCombinedOrderError(Task *task) {
	...
	this->onCancelOptionCombinedOrderError(data, error);
}
...
struct TraderApiWrap : TraderApi, wrapper < TraderApi >
{
	...
	virtual void onOptionCombinedOrderEvent(dict data, dict error_info) {
		PyLock lock;

		try {
			this->get_override("onOptionCombinedOrderEvent")(data, error_info);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onOptionCombinedTradeEvent(dict data) {
		PyLock lock;

		try {
			this->get_override("onOptionCombinedTradeEvent")(data);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onQueryOptionCombinedOrders(dict data, dict error_info, int request_id, bool is_last) {
		PyLock lock;

		try {
			this->get_override("onQueryOptionCombinedOrders")(data, error_info, request_id, is_last);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onQueryOptionCombinedOrdersByPage(dict data, int64_t req_count, int64_t order_sequence, int64_t query_reference, int request_id, bool is_last) {
		PyLock lock;

		try {
			this->get_override("onQueryOptionCombinedOrdersByPage")(data, req_count, order_sequence, query_reference, request_id, is_last);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onQueryOptionCombinedTrades(dict data, dict error_info, int request_id, bool is_last) {
		PyLock lock;

		try {
			this->get_override("onQueryOptionCombinedTrades")(data, error_info, request_id, is_last);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onQueryOptionCombinedTradesByPage(dict data, int64_t req_count, int64_t trade_sequence, int64_t query_reference, int request_id, bool is_last) {
		PyLock lock;

		try {
			this->get_override("onQueryOptionCombinedTradesByPage")(data, req_count, trade_sequence, query_reference, request_id, is_last);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onQueryOptionCombinedPosition(dict data, dict error_info, int request_id, bool is_last) {
		PyLock lock;

		try {
			this->get_override("onQueryOptionCombinedPosition")(data, error_info, request_id, is_last);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onQueryOptionCombinedStrategyInfo(dict data, dict error_info, int request_id, bool is_last) {
		PyLock lock;

		try {
			this->get_override("onQueryOptionCombinedStrategyInfo")(data, error_info, request_id, is_last);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};

	virtual void onCancelOptionCombinedOrderError(dict data, dict error_info) {
		PyLock lock;

		try {
			this->get_override("onCancelOptionCombinedOrderError")(data, error_info);
		} catch (error_already_set const &) {
			PyErr_Print();
		}
	};
